...
class TestTraxeed2IRIS(TestCase):


    def test_traxeed_to_iris_step(self):

        tr = TestRule()
        epcis_rule = tr.create_rule()
        curpath = os.path.dirname(__file__)
        data_path = os.path.join(curpath, 'data/tx_iris_comm_agg.xml')
    ...
    def _create_task(self, rule):
        task = models.Task()
        task.rule = rule
        task.name = 'unit test traxeed to iris'
        task.save()
        return task
    ...
    def create_rule(self):
        ...
        if not models.Rule.objects.filter(name=rule_name).exists():
            ...
            process_traxeed_step = models.Step.objects.create(
                ...
                order=2,
                step_class='quartet_integrations.traxeed.steps.TraxeedIRIS',
                rule=rule
            )
            ...
            models.StepParameter.objects.create(
                ...
                value='urn:epc:id:sgln:0529313.00000.0',
            ...
            models.StepParameter.objects.create(
                ...
                value='urn:epc:id:sgln:0457153.00000.0',
    ...
    def _create_output_criteria(self, endpoint, auth):
        try:
            EPCISOutputCriteria.objects.create(
                ...
                action='ADD',
                event_type='Object',
                biz_location='urn:epc:id:sgln:0457153.00000.0',
                end_point=endpoint,
